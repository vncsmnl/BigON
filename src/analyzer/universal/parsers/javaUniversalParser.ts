...
export class JavaUniversalParser {
  ...
  private buildFunctionNode(
    ...
    const callRegex = new RegExp(`\\b(?:this\\.)?${rawFn.name}\\s*\\(([^)]*)\\)`, 'g');
  ...
  private extractLoops(lines: { line: number; text: string }[]): UniversalLoopNode[] {
    ...
    for (const l of lines) {
      ...
      if (isFor || isWhile) {
        ...
        let explanation = this.messages.javaLoopLinear;
        ...
        if (
          ...
          explanation = this.messages.javaLoopLog;
        }
        ...
        if (stack.length > 0 && stack[stack.length - 1].loopKind === 'while') {
          if (
            ...
            stack[stack.length - 1].explanation = this.messages.javaWhileLog;
          }
        }
        if (l.text.includes('}')) {
          if (stack.length > 0) {
